...
workbook = xlrd.open_workbook(u'/Users/lin.tl/Downloads/M price.xlsx')
...
for i in range(1,nrows):
	for j in range(1,m_len+1):
		if len([x for x in (datasheet.row_values(i)[j::m_len]) if x > 0]) > 0:
			raw_mat[j-1].append([x for x in (datasheet.row_values(i)[j::m_len]) if x > 0][0])
		else:
			raw_mat[j-1].append(0)

ass_of_err = 0

for i in range(1,nrows):
	year, month, day, a, b, c = xlrd.xldate_as_tuple(datasheet.cell(i,0).value, 0)
	sheet1.write(i,0,str(year)+'/'+str(month)+'/'+str(day))
	if i == 1 or raw_mat[dic[month]][i-2] == 0:
		r_price.append(raw_mat[dic[month]][i-1])
		r_yield.append(-12345)
		ln_yield.append(-12345)
		print('Lacking some data.')
		ass_of_err = 1
		continue
	if raw_mat[dic[month]][i-1] == 0:
		r_price.append(raw_mat[dic[month]][i-1])
		r_yield.append(-12345)
		ln_yield.append(-12345)
		print('There is a data-lacking error.')
		ass_of_err = 1
		continue
	r_price.append(raw_mat[dic[month]][i-1])
...
output.save('/Users/lin.tl/Downloads/processed_M.xls')
